 ...
 */
class AccountsByCustomerActions(cassandra: Cassandra, simConf: SimConfig) extends BaseAction(cassandra, simConf) {
...
*/
  ...
  runQueries(Array(
    ...
    """.stripMargin,
   s"""
   | CREATE TABLE  IF NOT EXISTS  known_fraud_detection.accounts_by_customer (
   | email_id	text,
   | creation_timestamp  timestamp,
   | first_name text,
   | last_name text,
   | account_number text,
   | account_address frozen <address>,
   | PRIMARY KEY ( email_id	, creation_timestamp  ))
