 ...
 */
class DeviceMoveActivitiesActions(cassandra: Cassandra, simConf: SimConfig) extends BaseAction(cassandra, simConf) {
  ...
  private val writeQuery = QueryBuilder.insertInto(keyspace, table)
    ...
    .value("assigned_to_account", raw("?"))
    .value("assigned_from_account", raw("?"))
    .value("activity_code", raw("?")) 
    .value("activity_name", raw("?"))
  ...
  def writeActivities = {
    ...
    group(Groups.INSERT) {
      exec(cql("device_move_activities")
        .executePrepared(preparedStatement)
        .withParams(
            "${mac_address}",
          "${creation_timestamp}",
          "${assigned_to_account}",
          "${assigned_from_account}",
          "${activity_code}",
          "${activity_name}"
          )
...
*/
  def createTables(): Unit = {
   ...
   s"""
   | CREATE TABLE  IF NOT EXISTS  known_fraud_detection.device_move_activities (
     |  mac_address	text,
    |   creation_timestamp timestamp,
    |   assigned_to_account text,
    |   assigned_from_account text,
    |   activity_code text,
    |   activity_name text,
