...
class AccountInsertTestSimulation extends BaseSimulation {

  val simName = "knownFraudDetection"
  val scenarioName = "accountInsert"
  ...
  val accountActions = new AccountActions(cass, simConf)

  // Load feed for generating data
  val writeFeed = new AccountFeed(cass, simConf.getSimulationConfStr("keyspace"))

  // build scenario to run with feed and write action
  val writeScenario = scenario(scenarioName)
      .feed(writeFeed.getAccount)
      .exec(accountActions.writeAccount)
