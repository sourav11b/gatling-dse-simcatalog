...
class NetworkStatisticsByDeviceSimulation extends BaseSimulation {

  val simName = "knownFraudDetection"
  val scenarioName = "networkStatsInsert"
  ...
  val actions = new NetworkStatisticsByDeviceActions(cass, simConf )

  // Load feed for generating data
  val writeFeed = new NetworkStatisticsByDeviceFeed()

  // build scenario to run with feed and write action
  val writeScenario = scenario(scenarioName)
      .feed(writeFeed.getStats)
      .exec(actions.writeNetworkStats)
