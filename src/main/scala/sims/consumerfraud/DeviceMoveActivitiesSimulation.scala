...
import actions.consumerfraud.NetworkStatisticsByDeviceActions
import feeds.consumerfraud.NetworkStatisticsByDeviceFeed
import actions.consumerfraud.DeviceMoveActivitiesActions
import feeds.consumerfraud.DeviceMoveActivitesFeed


class DeviceMoveActivitiesSimulation extends BaseSimulation {

  val simName = "knownFraudDetection"
  val scenarioName = "deviceMoveInsert"
  ...
  val actions = new DeviceMoveActivitiesActions( cass, simConf )

  // Load feed for generating data
  val writeFeed = new DeviceMoveActivitesFeed()

  // build scenario to run with feed and write action
  val writeScenario = scenario(scenarioName)
      .feed(writeFeed.getActivities)
      .exec(actions.writeActivities)
